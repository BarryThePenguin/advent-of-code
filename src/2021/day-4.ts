...
function mark(numbers: MarkedNumber[], number: string) {
  for (let markedNumber of numbers.filter((markedNumber) => !markedNumber.marked)) {
...
function calculateScore(winner?: Winner) {
  if (winner) {
    const winningNumbers = winner.board.unmarkedNumbers.reduce((score, {number}) => score + parseInt(number, 10), 0)
    const winningNumber = parseInt(winner.winningNumber, 10)
...
class BingoBoard {
  ...
  get unmarkedNumbers() {
    ...
    for (let markedNumbers of this.rows) {
      unmarkedNumbers.push(...markedNumbers.filter((markedNumber) => !markedNumber.marked))
    }
  ...
  addNumbers(numbers: string) {
    ...
    for (let number of numbers.trim().split(/\s+/)) {
      rows.push({ number, marked: false })
    }
    
    for (let [index, markedNumber] of rows.entries()) {
  ...
  mark(number: string) {
    for (let row of this.markedNumbers) {
      mark(row, number)
    }
  }
}
...
class Bingo {
  ...
  playLast(numbers: string[]): Winner | undefined {
    let winningBoards = new Map<BingoBoard, Winner>()
